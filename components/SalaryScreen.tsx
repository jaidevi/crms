...
interface CalculationResult {
    ...
    summary: {
        ...
        baseWage: number; // Renamed from dailyWage to baseWage to represent either Daily or Monthly
        ratePerMeter: number;
...
const SalaryScreen: React.FC<SalaryScreenProps> = ({ employees, attendanceRecords, onUpdateEmployee, advances, onSavePayslip, onDeletePayslip, companyDetails, payslips }) => {
    ...
    const handleCalculate = () => {
        ...
        setEditableDeduction(String(Math.min(
            estimatedWageEarnings + (totalMetersProduced * ratePerMeter),
    ...
    const handleWageUpdate = async () => {
        if (!selectedEmployee || !result) return;

        setWageSaveState('saving');
        
        const newWage = Number(editableWage) || 0;
        const isMonthly = result.summary.isMonthly;

        const updatedEmployee = {
            ...selectedEmployee,
            dailyWage: isMonthly ? 0 : newWage,
            monthlyWage: isMonthly ? newWage : 0
        };

        try {
            await onUpdateEmployee(selectedEmployee.id, updatedEmployee);
            setWageSaveState('saved');
            setResult(prev => prev ? ({ ...prev, summary: { ...prev.summary, baseWage: newWage }}) : null);
            ...
            alert(`Failed to update wage. ${errorMessage}`);
        }
    };
    ...
    const handleFinalize = async () => {
        ...
        const wage = Number(editableWage) || 0;
        let wageEarnings = 0;
                                ...
                                <div className="bg-secondary-50 rounded-lg p-6 border border-secondary-200">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6">
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="flex justify-between items-center text-sm">
                                                <span>{result.summary.isMonthly ? 'Monthly Wage:' : 'Daily Wage:'}</span>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <input 
                                                        ...
                                                        className="w-24 p-1 text-right text-sm border rounded focus:ring-primary-500"
                                                    />
