...
const ShopMasterModal: React.FC<ShopMasterModalProps> = ({ onClose, onSave, existingClientNames, clientToEdit, processTypes, onAddProcessType }) => {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
        ...
        if (name === 'state') {
            ...
            setClient(prev => ({ ...prev, [name]: type === 'number' ? (value === '' ? 0 : Number(value)) : finalValue }));
        }
    ...
    const handleSave = () => {
        ...
        onSave({ ...client, name: client.name.trim(), processes: validProcessesForSave } as Client);
    };
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-start p-4 pt-10" role="dialog" aria-modal="true" aria-labelledby="client-master-modal-title">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl mx-4 animate-fade-in-down overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-6 space-y-6 overflow-y-auto flex-grow">
                    ...
                    <fieldset className="border border-gray-200 rounded-lg p-4 relative">
                        <legend className="text-sm font-semibold text-gray-900 bg-white px-2 absolute -top-2.5 left-4">Associated Processes</legend>
                        <div className="grid grid-cols-12 gap-4 px-3 py-2 bg-gray-50 rounded-md text-xs font-medium text-gray-500 tracking-wider uppercase pt-2">
                            <div className="col-span-6">Type of Process</div>
                            ...
                            {processes.map((item) => (
                                <div key={item.id} className="grid grid-cols-12 gap-4 items-center">
                                    ...
                                    <div className="col-span-3">
                                        <input type="number" value={item.rate || ''} min="0" step="0.01" onChange={e => updateProcessLineItem(item.id, { rate: Number(e.target.value) })} className={`${inputClasses} text-right`} aria-label="Rate" placeholder="0.00" />
                                    </div>
                        ...
                        <div className="mt-4">
                            <button onClick={addProcessLineItem} className="flex items-center text-sm font-medium text-blue-600 hover:text-blue-800">
