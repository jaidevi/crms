...
const DatePicker: React.FC<DatePickerProps> = ({ value, onChange, onClose, align = 'left' }) => {
    ...
    const dayClasses = (date: Date) => {
        ...
        if (!isCurrentMonth) {
            classes += 'text-secondary-300';
        } else {
            classes += 'cursor-pointer hover:bg-secondary-100 ';
            if (isSelected) {
                classes += 'bg-primary-600 text-white hover:bg-primary-700';
            } else if (isToday) {
                classes += 'border border-primary-500 text-primary-600 font-semibold';
            } else {
        ...
        <div ref={datepickerRef} className={`absolute top-full mt-2 w-72 bg-white border border-secondary-200 rounded-md shadow-lg z-50 animate-fade-in-down p-3 ${alignClass} no-print`}>
            <div className="flex items-center justify-between mb-3 px-1">
                ...
                <div className="flex items-center space-x-1">
                    <button onClick={() => changeMonth(-1)} className="p-1 rounded-full hover:bg-secondary-100 text-secondary-500" aria-label="Previous month">
                        <ChevronUpIcon className="w-5 h-5" />
                    </button>
                    <button onClick={() => changeMonth(1)} className="p-1 rounded-full hover:bg-secondary-100 text-secondary-500" aria-label="Next month">
                        <ChevronDownIcon className="w-5 h-5" />
                    </button>
                </div>
            </div>
            <div className="grid grid-cols-7 gap-y-2 text-center text-xs text-secondary-500 mb-2">
            ...
            <div className="flex justify-between items-center mt-3 pt-3 border-t border-secondary-200">
                <button onClick={handleClear} className="text-sm text-primary-600 hover:underline px-2 py-1 rounded">Clear</button>
                <button onClick={handleToday} className="text-sm text-primary-600 hover:underline px-2 py-1 rounded">Today</button>
            </div>
        </div>
    );
};
